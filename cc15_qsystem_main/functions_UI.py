...
# This block is equivalent to doing 'from main_[whatever main program is being run] import *'
if "__all__" in actualmod.__dict__:
    names = actualmod.__dict__["__all__"]
else:
    names = [x for x in actualmod.__dict__ if not x.startswith("_")]

globals().update({k: getattr(actualmod, k) for k in names})
...
class UIFunctions(MainWindow):
    ...
    def maximize_restore(self):
        ...
        if status == 0:
            ...
            self.ui.frame_top_btns.setStyleSheet("background-color: rgb(37, 39, 77)")
            ...
            self.ui.frame_top_btns.setStyleSheet("background-color: rgba(37, 39, 77, 200)")
    ...
    def selectMenu(getStyle):
        select = getStyle + ("QPushButton { border-right: 7px solid rgb(48, 51, 102); }")
        return select

    ## ==> DESELECT
    def deselectMenu(getStyle):
        deselect = getStyle.replace("QPushButton { border-right: 7px solid rgb(48, 51, 102); }", "")
        return deselect
